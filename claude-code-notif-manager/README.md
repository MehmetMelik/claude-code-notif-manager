...
The script downloads 6 sounds and writes `~/.claude/sounds/.theme`, which is sourced automatically by the shell scripts. To switch themes, run the script again with a different name, then `claude_sound_watcher_restart` to pick up the new sounds. Tech themes are generated locally via `ffmpeg` — no downloads needed.
